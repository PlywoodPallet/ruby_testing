...
describe FindNumber do
  ...
  describe '#game_over?' do
    context 'when guess and random_number are equal' do
      ...
      guess = 3
      let(:answer) { double('answer', value: guess) }
      subject(:game_over) { described_class.new(0,9,answer, guess) }
      ...
      it 'is game over' do
        # allow(random_number).to receive(:value).and equal(game_over.guess)
    ...
    context 'when guess and answer are not equal' do

      guess = 3
      let(:answer) { double('answer', value: 9) }
      subject(:game_over) { described_class.new(0, 9, answer, guess) }
