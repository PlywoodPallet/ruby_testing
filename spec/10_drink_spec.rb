...
describe Drink do
  describe '#initialize' do
    context 'when type is specified and ounces is default' do
      ...
      subject(:a_coffee) {described_class.new('coffee', 16)}

      # remove the 'x' before running this test
      it 'is your choice of beverage' do
        expect(a_coffee.type).to eq('coffee')
      end

      # remove the 'x' before running this test
      it 'has 16 ounces' do
        expect(a_coffee.ounces).to eq(16)
      end
    end
  end
  ...
  describe '#full?' do
    context 'when drink has 16 ounces or more' do
      # Create an explicit subject, using 'described_class' and your choice of
      # beverage type.
      subject(:kombucha) {described_class.new('kombucha', 16)}

      # remove the 'x' before running this test
      it 'is full' do
        expect(kombucha).to be_full
      end
    end
    ...
    context 'when drink has less than 16 ounces' do
      ...
      subject(:kombucha) {described_class.new('kombucha', 15)}

      # remove the 'x' before running this test
      it 'is not full' do
        expect(kombucha).not_to be_full
      end
    end
  end
end
